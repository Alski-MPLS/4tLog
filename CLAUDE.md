...
- Log Search's IP/port **range** filters (`(srcip>=x and srcip<=y)`, `(dstport>=a and dstport<=b)`) and the device-list source for the Log Search device picker are implemented per the assumptions in `docs/superpowers/specs/2026-07-25-phase3-log-search-design.md`'s "Open implementation-time checks" section, but have not yet been live-validated against a real FortiAnalyzer appliance — see that section for what remains to be confirmed.
